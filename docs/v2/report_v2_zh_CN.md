...
| 模型名称 | 模型类别 | 核心特性 |
|:--------|:--------|:--------|
| gemini-3.1-pro-preview | 通用闭源模型 | 海外主流通用大模型，支持长链推理 |
| Finix-X1 | 保险垂直领域模型 | 参数保险专属模型，闭源商用 |
| doubao-seed-2-0-lite-260215 | 通用闭源模型 | 国产主流大模型，推理版 |
| qwen3.5-plus | 通用闭源模型 | 国产主流大模型，推理版 |
| kimi-k2.5-nothinking | 通用开源模型 | 大规模开源模型，适配 openclaw 开源框架，非推理版 |
| doubao-seed-2-0-lite-260215-nothinking | 通用闭源模型 | 国产主流大模型，非推理版 |
| mimo-v2-pro | 通用闭源模型 | 国产金融领域大模型 |
| qwen3.5-plus-nothinking | 通用闭源模型 | 国产主流大模型，非推理版 |
| deepseek-r1-0528 | 通用开源模型 | 671B 参数开源大模型，支持长链推理 |
| qwen3-max | 通用闭源模型 | 适配开源框架，国产主流大模型 |
| deepseek-v3.2 | 通用开源模型 | 国产主流开源大模型 |
| Qwen3-235B-A22B-think | 通用开源模型 | 235B 参数开源大模型，推理版 |
| Qwen3-235B-A22B-instruct | 通用开源模型 | 235B 参数开源大模型，非推理版 |
| gpt-5.4 | 通用闭源模型 | 海外主流通用大模型，本次评测采用非推理版 |
| DianJin-R1 | 金融垂直领域模型 | 32B 参数开源金融大模型 |
| GPT-oss-120b | 通用开源模型 | 120B 参数开源大模型 |
| Fin-R1 | 金融垂直领域模型 | 7B 参数开源金融大模型 |
...
| **finixX1（蚂蚁保险大模型）** 🥈 | 87.23 | 🟠 84.61 | 🟠 87.54 | 🟠 85.69 | 🟠 86.43 | 🟢 91.87 |
...
- 垂直领域模型在业务相关理论上表现亮眼：finixX1 在保险法、保险产品等与业务落地强相关的细分项上得分领先，其中保险法细分项达 90.08 分，展现出领域训练对业务相关理论知识的精准强化效果。
...
- 保险垂直领域模型在严谨性维度展现出绝对领先优势，finixX1 以 91.87 分位列第一，在保险收益计算、金融数值计算等核心细分项上表现突出，其中保险收益计算达 93.25 分，充分体现了领域适配训练对保险场景数值计算、逻辑推理能力的强化效果。
...
- 垂直领域模型在合规维度保持领先，finixX1 以 85.69 分位列第一，在保险价值观、安全底线细分项上分别达 95.56 分、87.31 分，贴合保险行业强监管的特性，展现出领域模型对保险监管规则、合规价值观的深度适配。
- 保险文案合规仍是全行业最大合规风险点：所有模型在该细分项的最高分仅 87.68 分（finixX1），最低分仅 45.85 分，超 70% 的模型该细分项得分低于 80 分，即使是综合排名第一的 gemini-3.1-pro-preview 也仅 73.17 分，延续了 v1.0 发现的行业共性瓶颈，是当前 AIGC 在保险营销场景落地的核心风险。
...
- 通用模型与垂直领域模型形成双强格局，gemini-3.1-pro-preview 以 88.01 分位列第一，finixX1 以 87.54 分紧随其后，二者在投保、核保、核赔、理赔等保险全业务流程细分项上均表现突出，其中 finixX1 在保后操作、核赔相关细分项上分别达 94.78 分、93.38 分，展现出极强的业务场景适配能力。
- 小参数垂直模型实现对大模型的反超：32B 参数的 finixX1 在该维度排名第二，远超多款百亿、千亿参数的通用大模型，证明高质量的垂直领域训练，能够让中小参数模型在保险行业场景中实现能力突破，在当前算力成本大幅上涨的背景下，显著提升了保险行业大模型落地的性价比。
...
| 核心洞见 | 说明 |
|:--------|:--------|
| **垂直领域训练的价值持续验证，小参数模型实现高性价比突破** | 高质量的保险领域适配训练，依然是提升模型保险场景能力的核心路径。本次评测中，32B 参数的 finixX1 保险垂直模型综合排名第二，在保险严谨性、安全合规两大核心维度均位列第一，综合得分与千亿参数的通用大模型基本持平，甚至在核心业务场景实现反超。这一结果充分证明，在当前显卡与内存成本大幅上涨的行业背景下，通过精准的垂直领域训练，100B 以内的垂直中小参数模型完全能够满足保险行业的落地需求，为保险机构大模型私有化部署、轻量化应用提供了高性价比的解决方案。 |
